...
function renderBooks(items, name = "book", descr = false, modal = false) {
    if (modal === true) {
        const isInLibrary = (0, _localList.isBookInLibrary)(items);
        return `
  <div class="${name}" data-id="${items.id}">
             <div class="${name}__img"><img src="${thumbnail(items.volumeInfo)}" alt="" width="200"></div>
              <div class="${name}__wrapper">
                ...
                <button type="button" class="${name}__btn ${isInLibrary === false ? "" : "hidden"}"  data-name="add" data-id="${items.id}">add book from library</button>
      <button type="button" class="${name}__btn ${isInLibrary === true ? "" : "hidden"}" data-name="remove" data-id="${items.id}">remove book from library</button>
              </div>
            </div>`;
    }
...
},{"../image/no-image-placeholder.svg":"64l8W","./localList":"8IMTK","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"64l8W":[function(require,module,exports) {
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "isBookInLibrary", ()=>isBookInLibrary);
parcelHelpers.export(exports, "loadBtn", ()=>loadBtn);
parcelHelpers.export(exports, "loadModalBtn", ()=>loadModalBtn);
...
function addBook(e) {
    ...
    if (document.querySelector(".backdrop.show") !== null && document.querySelector(".backdrop.show").querySelector('button[data-name="add"]').getAttribute("data-id") === document.querySelector('button[data-name="add"]').getAttribute("data-id")) {
    ...
    if (libraryEl !== null) {
        libraryEl.innerHTML = "";
        libraryEl.innerHTML = (0, _renderBooks.renderBooks)(localArr, "library", false, false);
    }
    localArr = [];
}
function removeBook(e) {
    let localArr = JSON.parse(localStorage.getItem(LIBRARY_LIST));
    localArr.splice(localArr.findIndex((el)=>el.id === e.target.getAttribute("data-id")), 1);
    localStorage.setItem(LIBRARY_LIST, JSON.stringify(localArr));
    if (document.querySelector(".backdrop.show") && document.querySelector(".backdrop.show").querySelector('button[data-name="remove"]').getAttribute("data-id") === document.querySelector('button[data-name="remove"]').getAttribute("data-id")) {
...
function isBookInLibrary(book) {
    if (!temporary\u0421ell.some((el)=>el.id === book.id)) temporary\u0421ell.push(book);
...
function loadModalBtn(e) {
    if (document.querySelector(".backdrop.show") !== null) {
        const buttonsModalEl = {
            ...
            removeBookEl: document.querySelector(".backdrop.show").querySelector('button[data-name="remove"]')
        };
...
},{"./booksAPI":"lhp3x","./localList":"8IMTK","./renderBooks":"bhwKj","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9fKPd"], null, "parcelRequire78ed")
